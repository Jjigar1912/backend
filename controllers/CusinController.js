...
class CusinController {
  static async addCusin(req, res) {
    try {
      ...
      if (restaurant_id) {
        const existingOne = await CusinModel.findOne({
          name: { $regex: req.body.name, $options: "i" },
        });

        if (existingOne) {
          const existingData = await cusin_restaurant_model.findOne({
            restaurant_id,
            cusin_id: existingOne._id,
          });

          if (!existingData) {
            const newRecord = new cusin_restaurant_model({
              cusin_id: existingOne._id,
              restaurant_id,
          ...
          const newCusinRestaurant = new cusin_restaurant_model({
            cusin_id: data._id,
            restaurant_id,
            image: req.body.image,
          });
  ...
  static async displayCusin(req, res) {
    try {
      ...
      const data = await cusin_restaurant_model
        .find({ restaurant_id })
  ...
  static async removeCusin(req, res) {
    try {
      ...
      if (existingData) {
        const response = await cusin_restaurant_model.deleteOne({
          restaurant_id,
          cusin_id: req.body.cusin_id,
        });
  ...
  static async updateCusin(req, res) {
    try {
      ...
      const existed = await cusin_restaurant_model.findOne({
        restaurant_id,
        cusin_id: req.body.cusin_id,
      });
      ...
      if (existed) {
        ...
        if (!found) {
          ...
          const data = await cusin_restaurant_model.updateOne(
            { restaurant_id, cusin_id: req.body.cusin_id },
            { cusin_id: found._id, image: req.body.image }
          );
        ...
        return res
          .status(404)
          .json({ info: "You have not added this cusin . " });
      }
    } catch (error) {
      return res.status(500).json(error);
    }
